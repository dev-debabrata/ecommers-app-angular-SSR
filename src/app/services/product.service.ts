...
//     map((products) =>
//       products
//         .filter((p) => p.productType?.toLowerCase() === 'trending')
//         .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)),
//     ),
//   );
// }
...
//   const q = query(this.productsRef, where('productType', '==', type));
...
// getDiscountProducts(): Observable<Product[]> {
//   return this.getProducts().pipe(
//     map((products) => products.filter((p) => (p.discount || 0) >= 50)),
//   );
// }

//   getDiscountProducts(): Observable<Product[]> {
//   const q = query(this.productsRef, where('discount', '>=', 50));
//   return collectionData(q, { idField: 'id' }) as Observable<Product[]>;
// }
